...
from aiogram.filters import CommandStart, MagicData, Command
...
bot = Bot(token="8153857900:AAFguhLSz42mkFu1SZwtzzLrzHbF3Dq4lnw")
dp = Dispatcher()
...
# https://t.me/botusername?start=uuid4
@dp.message(
    CommandStart(deep_link=True),
    MagicData(F.command.args.cast(str).as_("receiver_id")),
)
async def deeplink_start(message: Message, receiver_id: str, state: FSMContext) -> None:
    # Сохраняем юзера-отправителя в базу
    id = str(uuid.uuid4())
    db.save_user(id, message.from_user.id)

    # Ищем юзера-получателя
    receiver = db.get_user_by_id(receiver_id)
    if receiver:
        # Если получатель существует, ставим состояние и ждём сообщение
        await state.set_state(AnonSG.wait_message)
...
async def default_start(message: Message) -> None:
    ...
    text = f"Привет! Твоя ссылка: t.me/{bot_info.username}?start={id}"
...
async def random_message(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(AnonSG.wait_message)
    await state.set_data({"receiver_id": receiver.id})
    await message.reply("Жду от тебя тайное сообщение")


@dp.message(AnonSG.wait_message)
